...
unsigned char player_check_floor(unsigned char f_lin, unsigned char f_col) {
  ...
  if (colint[SPR_P1] < 3) {
    ...
    index1 = index1 - 32;
    v2 = scr_map[index1];
    v2 = player_pick_deadly(v2);
    player_pick_item(v2, index1);
  } else {
    v0 = TILE_EMPTY;
  }
  ...
  if (colint[SPR_P1] > 0) {
    ...
    v1 = scr_map[index1];
    v1 = player_pick_deadly(v1);
    v1 = player_pick_item(v1, index1);
